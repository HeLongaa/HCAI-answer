...
function getBoundaryOffsetInMention(
  tag: Element,
  container: Node,
  offset: number,
) {
...
function getContentEditableBoundaryOffset(
  ...
  if (mentionTag) {
    ...
    if (!collapsed)
      return edge === 'start' ? mentionStart : mentionStart + mentionLength;
    const mentionOffset = getBoundaryOffsetInMention(
      mentionTag,
      container,
      offset,
    );
    return (
      mentionStart + (mentionOffset < mentionLength / 2 ? 0 : mentionLength)
    );
  }
  ...
  const element =
    container.nodeType === Node.ELEMENT_NODE ? (container as Element) : null;
  if (element) {
    let visibleOffset =
      element === root ? 0 : getVisibleOffsetBeforeNode(root, element);
  ...
  const findBoundary = (
    ...
    const walk = (current: Node): Boundary | null => {
      if (current.nodeType === Node.TEXT_NODE) {
        ...
        if (remaining <= node.length)
          return { type: 'offset', node, offset: remaining };
        remaining -= node.length;
        return null;
      }
...
function getFavoriteCollectionTasksForBatch(
  ...
  return favoriteTasks.filter((task) =>
    getTaskFavoriteCollectionIds(task).includes(collectionId),
  );
}
...
function agentImageMentionMatches(query: string, label: string) {
  ...
  return (
    ...
    normalizedLabel.replace(/^@/, '').includes(normalized)
  );
}
...
export default function InputBar() {
  ...
  const selectedSystemImageModelId = useStore(
    (s) => s.selectedSystemImageModelId,
  );
  const setSelectedSystemImageModelId = useStore(
    (s) => s.setSelectedSystemImageModelId,
  );
  ...
  const selectedFavoriteCollectionIds = useStore(
    (s) => s.selectedFavoriteCollectionIds,
  );
  const setSelectedFavoriteCollectionIds = useStore(
    (s) => s.setSelectedFavoriteCollectionIds,
  );
  const clearFavoriteCollectionSelection = useStore(
    (s) => s.clearFavoriteCollectionSelection,
  );
  const tasks = useStore((s) => s.tasks);
  const favoriteCollections = useStore((s) => s.favoriteCollections);
  const agentConversations = useStore((s) => s.agentConversations);
  const activeAgentConversationId = useStore(
    (s) => s.activeAgentConversationId,
  );
    ...
    return [
      {
        ...
        name: '全部',
        tasks: getFavoriteCollectionTasksForBatch(
          ALL_FAVORITES_COLLECTION_ID,
          tasks,
        ),
      },
  ...
  const filteredFavoriteCollectionCards = useMemo(() => {
    ...
    return favoriteCollectionCards.filter((collection) =>
  ...
  const handleSelectAllVisibleFavoriteCollections = useCallback(() => {
    setSelectedFavoriteCollectionIds(
      filteredFavoriteCollectionCards.map((collection) => collection.id),
    );
    ...
    try {
      ...
      const { successCount, failCount } = settings.zipDownloadRoutes.includes(
        ...
        ? await downloadImageEntriesAsZip(
            getTaskOutputImageZipEntries(selectedTasks),
            fileNameBase,
          )
  ...
  const handleDownloadSelectedFavoriteCollections = useCallback(async () => {
    ...
    const selectedCollections = favoriteCollectionCards.filter((collection) =>
    ...
    const useZipDownload = settings.zipDownloadRoutes.includes(
  ...
  const handleDeleteSelectedFavoriteCollections = useCallback(() => {
    const selectedIdSet = new Set(selectedFavoriteCollectionIds);
    const selectedCollections = favoriteCollections.filter((collection) =>
      selectedIdSet.has(collection.id),
    );
    ...
    const imageCount = new Set(
      tasks
        .filter((task) =>
          getTaskFavoriteCollectionIds(task).some((id) =>
            selectedCollectionIds.has(id),
          ),
        )
  ...
  const replaceImageTargetRef = useRef<{ index: number; id: string } | null>(
    null,
  );
    ...
    () =>
      settings.reuseTaskApiProfileTemporarily && reusedTaskApiProfileId
        ? (settings.profiles.find(
            (profile) => profile.id === reusedTaskApiProfileId,
  ...
  const activeAgentConversation =
    appMode === 'agent'
      ...
      : null;
  const activeAgentIsRunning = Boolean(
    activeAgentConversation?.rounds.some((round) => round.status === 'running'),
  );
  ...
  const systemImageModelOptions = systemImageModels.length
    ? systemImageModels.map((model) => ({
        label: model.display_name || model.site_model_id,
        value: model.site_model_id,
      }))
    : [
        {
          label: systemImageModelsLoading ? '加载中' : '无可用模型',
          value: '',
        },
      ];
  ...
  const agentOutputImageOptions = useMemo<AtImageOption[]>(() => {
    ...
    return getActiveAgentRounds(activeAgentConversation).flatMap((round) =>
      collectAgentRoundOutputImageSlots(round, tasks).flatMap(
        (imageId, imageIndex) => {
          if (!imageId) return [];
  ...
  const atImageOptions = atImageQuery
    ? [
        ...inputImages
          ...
          .filter((option) =>
            imageMentionMatches(atImageQuery.query, option.imageIndex),
          ),
        ...agentOutputImageOptions.filter((option) =>
          agentImageMentionMatches(atImageQuery.query, option.label),
        ),
      ]
    : [];
  ...
  const selectAtImageOption = useCallback(
    (option: AtImageOption) => {
      ...
      const query = getAtImageQuery(stripImageMentionMarkers(prompt), cursor, {
        length: atImageSourceCount,
      });
      ...
      const mentionText =
        option.type === 'input'
          ? getImageMentionLabel(option.imageIndex)
          : option.insertText;
  ...
  const insertPromptTextAtSelection = useCallback(
    (text: string) => {
      ...
      const promptStart = getPromptIndexFromVisibleIndex(
        prompt,
        selection.start,
      );
  ...
  const handleNLimitIncreaseAttempt = useCallback(
    (preventDefault: () => void) => {
      ...
      const effectiveValue = Number.isNaN(currentValue)
        ? params.n
        : currentValue;
  ...
  const openReplaceReferenceFilePicker = useCallback(
    (idx: number, imageId: string) => {
  ...
  const handleEditReferenceImage = useCallback(
    (img: (typeof inputImages)[number], idx: number, isMaskTarget: boolean) => {
      ...
      setConfirmDialog({
        title: '编辑参考图',
        message:
          '请选择这次要执行的操作。若不勾选下方的选项，则每次都询问；勾选后可在 **设置-习惯配置** 修改选择。',
  ...
  const handleReplaceFileUpload = async (
    ...
    try {
      ...
      if (
        currentImages.some(
          (item, itemIdx) => itemIdx !== targetIdx && item.id === image.id,
        )
      ) {
      ...
      showToast(
        `参考图替换失败：${err instanceof Error ? err.message : String(err)}`,
        'error',
      );
    }
  };
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    if (showAtImageMenu) {
      ...
      if (e.key === 'ArrowUp') {
        e.preventDefault();
        setAtImageMenuIndex(
          (idx) => (idx - 1 + atImageOptions.length) % atImageOptions.length,
        );
        return;
      }
      if ((e.key === 'Enter' && !e.shiftKey) || e.key === 'Tab') {
        e.preventDefault();
        selectAtImageOption(
          atImageOptions[atImageMenuIndex] ?? atImageOptions[0],
        );
        return;
      }
      if (e.key === 'Escape') {
  ...
  const adjustTextareaHeight = useCallback(() => {
    ...
    const placeholderEl = el.parentElement?.querySelector(
      '.prompt-placeholder',
    );
    ...
    // 3. 恢复平滑过渡并设置新目标高度
    el.style.transition =
      'height 150ms ease, border-color 200ms, box-shadow 200ms';
  ...
  useEffect(() => {
    ...
    const hasSentinel =
      last instanceof HTMLBRElement && last.dataset.sentinelBr === 'true';
  ...
  const paramControlClass =
    'hcai-playground-param-control px-3 py-1.5 border border-gray-200/60 dark:border-white/[0.08] bg-white/50 dark:bg-white/[0.03] hover:bg-white dark:hover:bg-white/[0.06] text-xs transition-all duration-200 shadow-sm';
  const paramControlDisabledClass =
    'hcai-playground-param-control px-3 py-1.5 border border-gray-200/60 dark:border-white/[0.08] bg-gray-100/50 dark:bg-white/[0.05] opacity-50 cursor-not-allowed text-xs transition-all duration-200 shadow-sm';
  ...
  const isBeforeMaskDropArea = (clientX: number) => {
    ...
    const maskEl = document.querySelector<HTMLElement>(
      '[data-input-image-index="0"]',
    );
  ...
  const renderImageThumb = (img: (typeof inputImages)[number], idx: number) => {
    ...
    const displaySrc =
      isMaskTarget && maskPreviewUrl ? maskPreviewUrl : img.dataUrl;
    ...
    const showDropAfter =
      imageDragOverIndex === inputImages.length &&
      isLast &&
      imageDragIndex !== idx;
          ...
          if (el) {
            ...
            if (
              document.execCommand(
                'insertHTML',
                false,
                getMentionTagHtml(getImageMentionLabel(idx)),
              )
            ) {
              syncPromptFromContentEditable();
              return;
  ...
  const renderParams = (cols: string) => (
    <div
      className={`hcai-playground-param-grid grid ${cols} gap-2 text-xs flex-1`}>
      {appMode === 'gallery' && (
        <label className="flex flex-col gap-0.5">
          <span className="text-gray-400 dark:text-gray-500 ml-1">模型</span>
          <Select
            value={selectedSystemImageModelId}
            onChange={(val) => setSelectedSystemImageModelId(String(val))}
            options={systemImageModelOptions}
            disabled={
              systemImageModelsLoading || systemImageModels.length === 0
            }
            className={
              systemImageModelsLoading || systemImageModels.length === 0
      ...
      <label
        className="hcai-playground-param-size relative flex flex-col gap-0.5"
      ...
      <label
        ...
        <input
          ...
          max={100}
          placeholder="0-100"
          className={`hcai-playground-param-control px-3 py-1.5 border border-gray-200/60 dark:border-white/[0.08] focus:outline-none text-xs transition-all duration-200 shadow-sm ${
            compressionDisabled
      ...
      <label
        ...
        <Select
          ...
          className={
            moderationDisabled ? paramControlDisabledClass : paramControlClass
          }
        />
        <ButtonTooltip
  ...
  const showFavoriteCollectionBatchBar =
    inCollectionOverview && selectedFavoriteCollectionIds.length > 0;
  const showTaskBatchBar =
    !showFavoriteCollectionBatchBar && selectedTaskIds.length > 0;
      ...
      <div
            ...
            <div className={batchActionBarClass}>
              ...
              <BatchActionButton
                ...
                selectedTaskIds.every(
                  (id) => tasks.find((t) => t.id === id)?.isFavorite,
                ) ? (
                  <svg
                    className="w-5 h-5"
                    fill="currentColor"
        ...
        <div
          ...
          <div className="relative grid">
                  ...
                  {atImageOptions.map((option, optionIndex) => (
                    <button
                      ...
                      {option.type === 'agent-output' && (
                        <span className="shrink-0 rounded bg-gray-100 px-1.5 py-0.5 text-[10px] text-gray-500 dark:bg-white/[0.06] dark:text-gray-400">
                          历史
                        </span>
                      )}
                    </button>
            ...
            {prompt.length === 0 && (
              <div
                className={`prompt-placeholder col-start-1 row-start-1 pointer-events-none pl-4 pr-10 py-3 text-sm leading-relaxed text-gray-400 dark:text-gray-500${
            ...
            <div className="hidden sm:flex items-end justify-between gap-3">
              ...
              <div className="flex gap-2 flex-shrink-0 mb-0.5">
                ...
                <div
                  ...
                  <ButtonTooltip
                    visible={
                      (activeAgentIsRunning || !hasSubmitApiConfig) &&
                      submitHover
                    }
