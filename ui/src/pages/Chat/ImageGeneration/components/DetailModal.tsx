...
export default function DetailModal() {
  ...
  const streamPreviewItems = useMemo(() => {
    ...
    const byIndex = new Map(
      slotEntries.map(([key, src]) => [Number(key), src]),
    );
  ...
  }, [imageIndex, persistedOutputLen, streamPreviewItems.length, task?.status]);
  ...
  const currentRawImageUrl = task?.rawImageUrls?.[imageIndex] || '';
  const currentOutputSource = currentOutputImageId || currentRawImageUrl;
  const outputSources = useMemo(() => {
    const outputImages = task?.outputImages ?? [];
    const rawImageUrls = task?.rawImageUrls ?? [];
    const count = Math.max(outputImages.length, rawImageUrls.length);
    return Array.from(
      { length: count },
      (_, index) => outputImages[index] || rawImageUrls[index],
    ).filter(Boolean);
  }, [task?.outputImages, task?.rawImageUrls]);
  const currentOutputKey =
    currentOutputImageId || (currentRawImageUrl ? `remote:${imageIndex}` : '');
  ...
  const canTogglePrompt = Boolean(task.prompt && task.prompt.length > 96);
  ...
  const outputLen = persistedOutputLen;
  const currentImageRatio = currentOutputKey
    ? imageRatios[currentOutputKey]
    : '';
  ...
  const taskProviderName =
    taskProvider === 'fal' ? 'fal.ai' : taskProvider ? 'OpenAI' : '未知';
  ...
  const handleDownloadCurrentOutput = async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!currentOutputSource || !task) return;
  ...
  const handleDownloadAllOutputs = async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!task || outputSources.length === 0) return;
    ...
    try {
      ...
      const result = settings.zipDownloadRoutes.includes('task-detail-all')
        ? await downloadImageEntriesAsZip(
            getImageZipEntries(outputSources, fileNameBase),
            fileNameBase,
          )
        : await downloadImageIds(outputSources, fileNameBase);
      if (result.successCount === 0) {
        showToast('下载失败', 'error');
  ...
  return (
    <Modal
      show
      onHide={() => setDetailTaskId(null)}
      centered
      dialogClassName="hcai-detail-modal hcai-size-picker-modal hcai-subscription-dialog"
      contentClassName="hcai-detail-modal-panel hcai-size-picker-panel">
      <Modal.Body
        data-no-drag-select
        className="hcai-detail-modal-body hcai-size-picker-body">
        <button
        ...
        <div
          ref={modalRef}
          className="hcai-detail-modal-layout relative flex"
          onClick={(e) => e.stopPropagation()}>
          {/* 左侧：图片 */}
          <div className="hcai-detail-modal-preview h-64 bg-gray-100 dark:bg-black/20 relative flex items-center justify-center flex-shrink-0 min-h-[16rem]">
            {task.status === 'done' &&
              outputLen > 0 &&
                  ...
                  <img
                    ...
                    onLoad={(e) => {
                      ...
                      if (
                        ...
                        setImageRatios((prev) => ({
                          ...prev,
                          [currentOutputKey]: formatImageRatio(
                            image.naturalWidth,
                            image.naturalHeight,
                          ),
                        }));
                    ...
                    onClick={() => {
                      if (!currentOutputSource) return;
                      setLightboxImageId(currentOutputSource, outputSources);
                    }}
                    alt=""
                  />
                  <div
                    data-selectable-text
                    className="absolute left-4 top-[15px] flex items-center gap-1.5">
          ...
          {/* 右侧：信息 */}
          <div className="hcai-detail-modal-info p-5 overflow-y-auto overscroll-contain flex flex-col">
            <div data-selectable-text className="flex-1">
              <div className="flex items-center gap-1.5 mb-2">
                <h3 className="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider">
                  输入内容
                </h3>
                {canTogglePrompt && !showPendingPrompt && (
                  <button
                    type="button"
                    className="hcai-detail-modal-prompt-toggle"
              ...
              {showPendingPrompt ? (
                ...
                <button
                  type="button"
                  className={`hcai-detail-modal-prompt text-left text-sm text-gray-700 dark:text-gray-300 leading-relaxed whitespace-pre-wrap mb-4 ${
                    canTogglePrompt ? 'is-toggleable' : ''
            ...
            {/* 操作按钮 */}
            <div className="hcai-detail-modal-actions flex flex-wrap gap-2 pt-4 border-t border-gray-100 dark:border-white/[0.08]">
              <button
                onClick={handleReuse}
                className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-500/20 transition text-sm font-medium whitespace-nowrap">
                <svg
              ...
              <button
                onClick={handleEdit}
                disabled={!outputLen}
                className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl bg-green-50 dark:bg-green-500/10 text-green-600 dark:text-green-400 hover:bg-green-100 dark:hover:bg-green-500/20 disabled:opacity-40 disabled:cursor-not-allowed transition text-sm font-medium whitespace-nowrap">
              ...
              <button
                onClick={(e) => {
                  if (outputLen > 1) {
                    void handleDownloadAllOutputs(e);
                  } else {
                    void handleDownloadCurrentOutput(e);
                  }
                }}
                disabled={!outputLen}
                className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl bg-cyan-50 dark:bg-cyan-500/10 text-cyan-600 dark:text-cyan-300 hover:bg-cyan-100 dark:hover:bg-cyan-500/20 disabled:opacity-40 disabled:cursor-not-allowed transition text-sm font-medium whitespace-nowrap">
                <DownloadIcon className="w-4 h-4 flex-shrink-0" />
                {outputLen > 1 ? '下载全部' : '下载图片'}
              </button>
              <button
                onClick={handleDelete}
                className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-500/20 transition text-sm font-medium whitespace-nowrap">
