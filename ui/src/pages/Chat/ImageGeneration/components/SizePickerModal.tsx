...
import type { AiImageModelSizeOption } from '@/common/interface';
import {
  calculateImageSize,
  normalizeImageSize,
  parseRatio,
...
interface Props {
  ...
  onClose: () => void;
  allowAuto?: boolean;
  fixedOptions?: AiImageModelSizeOption[];
}
...
export default function SizePickerModal({
  ...
  const normalizedCurrentSize = normalizeImageSize(currentSize);
  const currentFixedOption = hasFixedOptions
    ? fixedOptions.find((option) => option.value === normalizedCurrentSize)
    : undefined;
  ...
  const [mode, setMode] = useState<Mode>(() => {
    ...
    if (hasFixedOptions) return 'ratio';
  ...
  const [tier, setTier] = useState<SizeTier>(
    (currentFixedOption?.tier as SizeTier | undefined) ??
      currentPreset?.tier ??
      '1K',
  );
  const [ratio, setRatio] = useState(
    currentFixedOption?.aspect_ratio ??
      currentPreset?.ratio ??
      (allowAuto ? '1:1' : '4:3'),
  );
  ...
  useEffect(() => {
    ...
    const matched = fixedOptions.find(
      (option) => option.value === normalizedCurrentSize,
    );
    const nextOption = matched ?? fixedOptions[0];
    if (!nextOption) return;
    setTier(nextOption.tier as SizeTier);
    setRatio(nextOption.aspect_ratio);
  }, [fixedOptions, hasFixedOptions, normalizedCurrentSize]);
  ...
  const previewSize = useMemo(() => {
    ...
    if (mode === 'ratio') {
      if (hasFixedOptions) {
        return (
          fixedOptions.find(
            (option) => option.tier === tier && option.aspect_ratio === ratio,
          )?.value ??
          fixedOptions.find((option) => option.tier === tier)?.value ??
          fixedOptions[0]?.value ??
          ''
        );
      }
  ...
  const isClamped = useMemo(() => {
    if (hasFixedOptions) return false;
  ...
  const fixedTierOptions = useMemo(() => {
    if (!hasFixedOptions) return TIERS;
    return TIERS.filter((item) =>
      fixedOptions.some((option) => option.tier === item),
    );
  }, [fixedOptions, hasFixedOptions]);

  const fixedRatioOptions = useMemo(() => {
    if (!hasFixedOptions) return RATIOS;
    const seen = new Set<string>();
    return fixedOptions
      .filter((option) => option.tier === tier)
      .flatMap((option) => {
        if (seen.has(option.aspect_ratio)) return [];
        seen.add(option.aspect_ratio);
        return [{ label: option.aspect_ratio, value: option.aspect_ratio }];
      });
  }, [fixedOptions, hasFixedOptions, tier]);
  ...
  const selectPresetRatio = (value: string) => {
    ...
    window.requestAnimationFrame(() =>
      scrollContentTo(ratioSectionRef.current),
    );
  };
  ...
  const tabClass = (active: boolean) =>
    `hcai-size-picker-tab ${active ? 'is-active' : ''}`;
  const buttonClass = (active: boolean) =>
    `hcai-size-picker-choice px-3 py-2 transition ${active ? 'is-active' : ''}`;
      ...
      <Modal.Body className="hcai-size-picker-body">
        ...
        <div>
          ...
          <div ref={contentRef} className="hcai-size-picker-content pr-1 -mr-1">
            {mode === 'auto' && (
              <div className="flex h-full animate-fade-in items-center justify-center pt-8 pb-4 text-center">
                <div>
                  ...
                  <h4 className="hcai-size-picker-empty-title text-sm">
                    自动尺寸
                  </h4>
            ...
            {mode === 'ratio' && (
              <div className="space-y-5 animate-fade-in">
                <section ref={ratioSectionRef}>
                  <div className="hcai-size-picker-section-title mb-2">
                    基准分辨率
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    {fixedTierOptions.map((item) => (
                      <button
                        key={item}
                ...
                {!hasFixedOptions && ratio === 'custom' && (
                  <label
                    ref={customRatioFieldRef}
                    className="block animate-fade-in scroll-mt-3">
                    <span className="hcai-size-picker-field-label mb-2 block">
                      输入自定义比例
                    </span>
                    <input
                      value={customRatio}
            ...
            {!hasFixedOptions && mode === 'resolution' && (
              <div className="space-y-5 animate-fade-in">
                <section>
                  <div className="hcai-size-picker-section-title mb-4">
                    输入具体像素值
                  </div>
                  <div className="flex items-center gap-4">
                    <label className="flex-1">
                      <span className="hcai-size-picker-field-label mb-1.5 block">
                        宽度 (Width)
                      </span>
                      <input
                        type="number"
