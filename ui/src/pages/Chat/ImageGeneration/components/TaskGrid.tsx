...
export default function TaskGrid() {
  ...
  useEffect(() => {
    ...
    const startDragScroll = (direction: -1 | 1) => {
      ...
      dragScrollIntervalRef.current = window.setInterval(() => {
        window.scrollBy({ top: direction * 15, behavior: 'auto' });
      }, 16);
    };

    const endSelection = (
      clearEmptySurfaceClick = false,
      suppressClick = false,
    ) => {
    ...
    const handleDocumentMouseMove = (e: MouseEvent) => {
      ...
      const distance = Math.hypot(
        point.pageX - start.pageX,
        point.pageY - start.pageY,
      );
    ...
    const handleDocumentScroll = () => {
      ...
      const point = getPagePoint(
        lastClientPoint.current.x,
        lastClientPoint.current.y,
      );
      const start = dragStart.current;
      setSelectionBox({
        startPageX: start.pageX,
    ...
    const handleDocumentWheel = (e: WheelEvent) => {
      ...
      if (now - lastToastTimeRef.current > 3000) {
        ...
        useStore
          .getState()
          .showToast(`松开 ${keyName} 键使用滚轮，或拖至边缘自动滚动`, 'info');
      }
    };
  ...
  if (!filteredTasks.length) {
    return (
      <div className="hcai-task-empty-state text-center py-20 text-gray-400 dark:text-gray-500">
  ...
  return (
    <div ref={rootRef} data-task-grid-root className="relative min-h-[50vh]">
      <div
        ref={gridRef}
        className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,20rem),1fr))] gap-4 pb-10">
      ...
      {selectionBox && (
        <div
          ...
          style={{
            left:
              Math.min(selectionBox.startPageX, selectionBox.currentPageX) -
              window.scrollX,
            top:
              Math.min(selectionBox.startPageY, selectionBox.currentPageY) -
              window.scrollY,
            width: Math.abs(
              selectionBox.currentPageX - selectionBox.startPageX,
            ),
            height: Math.abs(
              selectionBox.currentPageY - selectionBox.startPageY,
            ),
          }}
        />
      )}
