...
export default function Lightbox() {
  ...
  useEffect(() => {
    ...
    if (isRemoteImageSource(imageId)) {
      setSrc(resolveSystemImageUrl(imageId));
      return () => {
        cancelled = true;
      };
    }
  ...
  useEffect(() => {
    ...
    const taskWithMask = tasks.find(
      (t) => t.maskTargetImageId === lightboxImageId && t.maskImageId,
    );
  ...
  // 导航
  const currentIndex = lightboxImageId
    ? lightboxImageList.indexOf(lightboxImageId)
    : -1;
  ...
  const goTo = useCallback(
    (idx: number) => {
      ...
      const wrapped =
        ((idx % lightboxImageList.length) + lightboxImageList.length) %
...
function LightboxInner({
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      'image-context-menu-dismiss-lightbox-click',
      suppressClick,
    );
    return () =>
  ...
  useEffect(() => {
    ...
    const onTouchStart = (e: TouchEvent) => {
      if (e.touches.length === 2) {
        ...
        touchStartedOnControlRef.current =
          e.target instanceof Element && Boolean(e.target.closest('button'));
    ...
    const onTouchMove = (e: TouchEvent) => {
      if (pinchRef.current.active && e.touches.length === 2) {
        ...
        apply(
          ns,
          p.midX - r * (p.midX - p.startTx),
          p.midY - r * (p.midY - p.startTy),
        );
        ...
        if (
          touchIntentRef.current === 'none' &&
          (absX > SWIPE_INTENT_THRESHOLD || absY > SWIPE_INTENT_THRESHOLD)
        ) {
          touchIntentRef.current =
            absX > absY ? 'horizontal-swipe' : 'vertical-move';
  ...
  return createPortal(
    <div
      ref={containerRef}
      data-lightbox-root
      className="fixed inset-0 z-[1300] flex items-center justify-center select-none"
      style={{
