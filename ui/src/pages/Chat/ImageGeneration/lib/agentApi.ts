...
function createAgentTools(
  ...
  const tools: Array<Record<string, unknown>> = [
    createImageTool(params, streamImages, streamPartialImages, maskDataUrl),
  ];
          ...
          items: {
            ...
            properties: {
              id: {
                type: 'string',
                description:
                  'Short stable identifier for this image, e.g. "slide_2_problem", "scene_3".',
              },
              prompt: {
                type: 'string',
                description:
    ...
    parameters: {
      ...
      properties: {
        reason: {
          type: 'string',
          description:
            'Brief explanation of why another round is needed and what will be generated next.',
        },
      },
      required: ['reason'],
      additionalProperties: false,
...
type ResponseTextAnnotation = NonNullable<
  NonNullable<ResponsesOutputItem['content']>[number]['annotations']
>[number];

function applyUrlCitations(
  text: string,
  annotations: ResponseTextAnnotation[] | undefined,
) {
  ...
  for (const citation of citations) {
    ...
    const label =
      text.slice(start, end) || citation.title || citation.url || 'source';
...
function getStreamEventErrorMessage(
  ...
  if (type?.endsWith('.failed'))
    return getStringValue(event, 'message') ?? 'Agent 流式请求失败';
  return null;
}

async function getAgentProxyErrorMessage(response: Response) {
  const fallback = `Agent 请求失败：${response.status}`;
  try {
...
function extractImages(
  ...
  for (const item of payload.output ?? []) {
    const image = extractImageFromOutputItem(item, fallbackMime);
...
function getStreamResponsePayload(
  event: Record<string, unknown>,
...
async function parseAgentStreamResponse(
  ...
  const publishOutputItems = (items: ResponsesOutputItem[]) => {
    for (const item of items) {
      const index = item.id
        ? outputItems.findIndex((existing) => existing.id === item.id)
        : -1;
  ...
  const publishWebSearchStatus = (
    ...
    const current =
      index >= 0 ? outputItems[index] : { id, type: 'web_search_call' };
  ...
  const payload: ResponsesApiResponse | null =
    completedPayload ?? (outputItems.length ? { output: outputItems } : null);
...
export async function callAgentResponsesApi(opts: {
  ...
  try {
    ...
    const response = await fetch('/answer/api/v1/ai-agent/responses', {
      method: 'POST',
      headers: {
        Authorization: Storage.get(LOGGED_TOKEN_STORAGE_KEY) || '',
    ...
    if (!response.ok) {
      throw new Error(await getAgentProxyErrorMessage(response));
    }

    if (stream && isEventStreamResponse(response)) {
...
export async function callAgentConversationTitleApi(opts: {
  ...
  const { settings, model, prompt, imageDataUrls, signal } = opts;
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 600 * 1000);
  const abortFromCaller = () => controller.abort();
  if (signal?.aborted) controller.abort();
...
export function parseSingleImageCallArguments(
  args: string,
): { prompt: string } | null {
  try {
  ...
  try {
    const referenceMapping =
      referenceImageDataUrls.length > 0
        ? `Attached reference images correspond to these ids, in order: ${
            (referenceIds ?? [])
              .map((id) => `<ref id="${id}" />`)
    ...
    tool.action = referenceImageDataUrls.length > 0 ? 'auto' : 'generate';
    ...
    if (!response.ok) {
      return {
        batchItemId,
        image: null,
        error: await getAgentProxyErrorMessage(response),
      };
    }
    ...
    if (stream && isEventStreamResponse(response)) {
      ...
      await readJsonServerSentEvents(
        ...
        async (event) => {
          ...
          if (type === 'response.image_generation_call.partial_image') {
            ...
            if (b64) {
              await onPartialImage?.({
                image: normalizeBase64Image(b64, mime),
                partialImageIndex: getNumberValue(
                  event,
                  'partial_image_index',
                ),
              });
          ...
          if (type === 'response.completed' || isRecordValue(event.response)) {
            const payload = getStreamResponsePayload(event);
            if (payload) rawResponsePayload = JSON.stringify(payload, null, 2);
    ...
    const payload = (await response.json()) as ResponsesApiResponse;
    const image = extractImages(payload, mime)[0] ?? null;
    if (image) await onImageToolCompleted?.(image);
    return {
      batchItemId,
      image,
